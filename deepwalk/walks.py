...
def write_walks_to_disk(G, filebase, num_paths, path_length, alpha = 0, rand = random.Random(0), num_workers = cpu_count(), always_rebuild = True):
    ...
    if num_paths <= num_workers:
        ...
        # note that this is inefficient since not all workers are used on some cases
        # filter returns elements if the condition (lambda) is satisfied
        paths_per_worker = [len(filter(lambda z: z != None, [y for y in x])) for x in graph.grouper(int(num_paths / num_workers) + 1, range(1, num_paths + 1))]
